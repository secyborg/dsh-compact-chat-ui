...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		var LOCALE_ZH = {
			...
			lineHeight: "行高",
			spacing: "块间距",
			fontSizeHint: "11–18，应用原始值 16",
			lineHeightHint: "1.2–2.0，应用原始值 1.00",
			spacingHint: "0.25–1.5，应用原始值 1.00",
			unsaved: "未保存",
			discard: "取消",
			save: "保存",
			overridden: "已覆盖",
		...
		var LOCALE_EN = {
			...
			fontSizeHint: "11–18; stock is 16",
			lineHeightHint: "1.2–2.0; stock is 1.00",
			spacingHint: "0.25–1.5; stock is 1.00",
			unsaved: "Unsaved",
			discard: "Discard",
			save: "Save",
		...
		var CARD_CSS = ".ccui_card{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);border-radius:12px;list-style:none;transition:border-color .16s,background .16s}" +
			...
			".ccui_input{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-layer-3);height:34px;font:inherit;color:var(--dsw-alias-label-primary);border-radius:8px;padding:0 12px;font-size:13px;line-height:1.5;box-sizing:border-box;width:130px}" +
					...
					FIELDS.map(function (field) {
						...
						return h("div", { className: "ccui_field", key: field.name },
							h("div", { className: "ccui_fhead" },
								h("span", { className: "ccui_label" }, t(field.label)),
							...
							h("p", { className: "ccui_hint" }, t(field.hint))
						);
					}),
