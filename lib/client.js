...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function SettingsCard() {
			var t = localeT; // self-contained: section components get no locale prop
			var openState = react.useState(true); // a whole settings page now — open by default
		...
		function apply(ctx) {
			...
			// Settings section: DSH Desktop 2.0.2 turned "settings.plugin.item"
			// into a keyed slot gated on Host-served settings namespaces, which
			// a client-only plugin cannot join. "settings.section" (the slot
			// better-sidebar uses) takes a plain id and renders a whole
			// settings page, so the typography card lives there now.
			try {
				...
				ctx.slots.inject("settings.section", function () {
					return ctx.slots.register({
						...
						id: "compact-chat-ui",
						order: 120,
						label: function () { return localeT("navLabel"); },
						inject: function () { return {}; }
					}, SettingsCard);
				}, "dsh-compact-chat-ui: settings section");
			} catch (error) {
				console.warn("[dsh-compact-chat-ui] settings section skipped:", error);
			}
		}
